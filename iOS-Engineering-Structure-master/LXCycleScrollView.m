...
@interface LXCycleScrollView () <UIScrollViewDelegate>
//轮播的图片数组
...
//创建用来缓存图片的文件夹
+ (void)initialize {
    cache = [[NSSearchPathForDirectoriesInDomains(NSCachesDirectory, NSUserDomainMask, YES) lastObject] stringByAppendingPathComponent:@"LXCycleScrollView"];
    BOOL isDir = NO;
    ...
    for (int i = 0; i < imageArray.count; i++) {
        if ([imageArray[i] isKindOfClass:[UIImage class]]) {
            ...
            if (_placeholderImage) {
                ...
                UIImage *placeholderImage = [UIImage imageNamed:@"LXCycleScrollView"];
